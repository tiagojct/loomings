...
- The service worker is now registered by a versioned URL (`sw.js?v=…`)
  and served with `no-cache`. The CDN in front of loomings.tiagojacinto.eu
  caches JavaScript for a year, so after 2.0.1 the edge kept handing out
  the 2.0.0 worker.
